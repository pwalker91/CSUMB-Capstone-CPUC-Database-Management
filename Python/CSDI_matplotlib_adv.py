...
class CSDI_MPL():
    ...
    def barGraph(self, data):
        ...
        for keys, vals in data.items():
            if keys == 'AT&T':
                graph1 = ax.bar(ind, statistics.pvariance(vals) , width, color = 'b', yerr = statistics.pstdev(vals))
            if keys == 'Verizon':
                graph2 = ax.bar(ind+width, statistics.pvariance(vals), width, color = 'r', yerr = statistics.pstdev(vals))
            if keys == 'Sprint':
                graph3 = ax.bar(ind+(2*width), statistics.pvariance(vals), width, color = 'y', yerr = statistics.pstdev(vals))
            if keys == 'T-Mobile':
                graph4 = ax.bar(ind+ (3 * width), statistics.pvariance(vals), width, color = 'm', yerr = statistics.pstdev(vals))
        ax.set_ylabel('Speeds')
        ax.set_title('Variance of Speeds with standard deviation grouped by Provider')
        ax.set_xticks(ind/4)
        ...
        autolabel(graph3)
        autolabel(graph4)
        plt.savefig("VarianceSpeedsProviders.png")
        filepath = os.path.join(os.getcwd(), "VarianceSpeedsProviders.png")
        plt.show()
        print (filepath)
        return filepath
