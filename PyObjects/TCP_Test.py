...
VARIABLES:
  ...
  NOT INHERITED
    ThreadsByNum        List, holding all of the TCP_Threads in this test, accessed by
    WindowSize          Integer, the size of TCP window to be used in this test
    ThreadNumbers       List of Strings, the numbers that were used by the threads in this test (e.g. ['3','4','5','6'])
...
FUNCTIONS:
  INITIALIZATION:
    __init__ - Used to initialize an object of this class
        ...
        OUTPUTS-    none
    parseThreads - Given the data stored in self._text, parses the Iperf output into individual TCP_Threads
        INPUTS-     self:   reference to the object calling this method (i.e. Java's THIS)
        OUTPUTS-    none
    checkForOutputErrors - After all of the threads have been initialized, this function checks for
  ...
  THREAD VALUE GETTERS
    get_TCPSumValues - Creating an array of the sum of each 1 second interval of all 4 thread's speed or size
        INPUTS-     self:               reference to the object calling this method (i.e. Java's THIS)
    ...
    get_ThreadsValues - Creating an array of array of speed for each 1 second interval for each thread in the test
        INPUTS-     self:               reference to the object calling this method (i.e. Java's THIS)
                    direction:          String, threads of specified direction (Up or Down) that will be summed
                    attribute:          String, the attribute of the measurement we wish to sum up (speed or size)
        OUTPUTS-    threads_summed:     an array containing values representing the sum of each 4 threads' speed
  ...
  FOR STAT ANALYSIS (append to CSV)
    ...
    get_csvStatValues - Creates 4 value long array, to be returned to the SpeedTestFile object.
    ...
    get_csvQualValues - Creates a List of 4 values, the quality value of the upload and download
                        threads in this test
        INPUTS-     self:       reference to the object calling this method (i.e. Java's THIS)
        OUTPUTS-    toReturn:   Array, 4 values corresponding to this test's time and data quality for Up and Down direction
...
from PyObjects.TCP_Thread import TCP_Thread
#END IMPORTS
...
# Intialization Functions ----------------------------------------------------------------
    ...
    # DESC: Given the data stored in self._text, parses the Iperf output into individual TCP_Threads
    ...
    # DESC: After all of the threads have been initialized, this function checks for
    #       some special cases of output errors, i.e. final measurement is 0.0 - 0.0 sec,
    #       final msmt download speed is in the order to 10^16, or 10 Quadrillion Kbits/sec
...
# Thread value Getters -------------------------------------------------------------------
    ...
    def get_TCPSumValues(self, direction="DOWN", attribute="Speed"):
    ...
    # DESC: This returns the threads of the given direction in this test
    #       as an array of values
...
# For statistical analysis ----------------------------------------------------------------------
    ...
    # DESC: Creates a List of the 2 default values, the sum of the final measurement's
    #       speeds from all threads.
    ...
    def get_csvStatValues(self):
        ...
        if not self.ContainsErrors:
            ...
            upThread = self.get_TCPSumValues(direction="UP", attribute="Speed")
            downThread = self.get_TCPSumValues(direction="DOWN", attribute="Speed")
    ...
    # DESC: This function calculates the quality of the TCP connection.
